...
class ChatBot {
    constructor() {
        ...
        this.selectedProvider = 'auto'; // Default to auto mode
        try {
            this.hasUsedChatbot = localStorage.getItem('chatbot-used') === 'true';
            this.selectedProvider = localStorage.getItem('chatbot-provider') || 'auto';
        } catch (e) {
    ...
    setupModelSwitcher() {
        ...
        modelOptions.forEach(option => {
            if (option.dataset.provider === this.selectedProvider) {
        ...
        modelOptions.forEach(option => {
            option.addEventListener('click', () => {
                ...
                // Save preference
                try {
                    localStorage.setItem('chatbot-provider', this.selectedProvider);
                } catch (e) {
                    console.warn('Could not save provider preference');
                }
                
                // Show brief feedback
                this.showProviderChangeMessage(option.dataset.provider);
            });
        });
    }

    showProviderChangeMessage(provider) {
        const providerConfig = {
            auto: { name: 'AutoPilot', icon: '🤖', description: 'Smart mode activated' },
            gemini: { name: 'Quick Draw', icon: '⚡', description: 'Fast mode activated' },
            perplexity: { name: 'Deep Dive', icon: '🔍', description: 'Research mode activated' }
        };
        ...
        systemDiv.innerHTML = `
            <div class="message-content">
                <p><span class="model-icon">${config.icon}</span> ${config.description}</p>
            </div>
        `;
    ...
    async sendMessage(message) {
        ...
        console.log('🚀 Sending message to backend:', message, 'Provider:', this.selectedProvider);
    ...
    addMessage(message, sender, metadata = null) {
        ...
        if (sender === 'bot') {
            ...
            if (metadata) {
                const metadataDiv = this.createMetadataElement(metadata);
    ...
    createMetadataElement(metadata) {
        const providerConfig = {
            auto: { name: 'AutoPilot', icon: '🤖', color: 'purple' },
            gemini: { name: 'Quick Draw', icon: '⚡', color: 'yellow' },
            perplexity: { name: 'Deep Dive', icon: '🔍', color: 'blue' }
        };
        ...
        if (metadata.provider) {
            const config = providerConfig[metadata.provider] || { name: metadata.provider, icon: '🤖', color: 'gray' };
            metadataHTML += `<span class="metadata-badge provider-badge provider-${config.color}">
                <span class="model-icon">${config.icon}</span> via ${config.name}
            </span>`;
        }
        
        // Response time
        if (metadata.responseTime) {
            const timeStr = metadata.responseTime.toFixed(1);
            metadataHTML += `<span class="metadata-badge time-badge">${timeStr}s</span>`;
        }
        
        metadataHTML += '</div>';
        
        // Sources (for Deep Dive/Perplexity)
        if (metadata.sources && metadata.sources.length > 0) {
            metadataHTML += `
                <div class="sources-section">
                    <button class="sources-toggle">
                        <span class="model-icon">🌐</span> Sources (${metadata.sources.length})
                        <i class="fas fa-chevron-down"></i>
                    </button>
                    <div class="sources-list" style="display: none;">
                        ${metadata.sources.map((source, index) => {
                            const domain = this.extractDomain(source);
                            return `<a href="${source}" target="_blank" rel="noopener noreferrer" class="source-link">
                                ${index + 1}. ${domain}
                            </a>`;
                        }).join('')}
                    </div>
        ...
        // Add toggle functionality for sources
        const sourcesToggle = metadataDiv.querySelector('.sources-toggle');
        if (sourcesToggle) {
            sourcesToggle.addEventListener('click', () => {
                const sourcesList = metadataDiv.querySelector('.sources-list');
                const chevron = sourcesToggle.querySelector('.fa-chevron-down');
